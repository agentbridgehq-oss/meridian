/** Public accept/revise + Ken go-ahead. Default STOP. */
import {
  getJobByToken,
  customerRespond,
  executeJob,
...
<div class="card">
  <p>${p.summary || 'Voice + Sales + Booking agent. Meridian reviews before go-live.'}</p>
...
<form id="f">
  ...
  <p class="muted">Payment is Stripe. Ken must still approve go-live. We do not invent prices.</p>
  <button type="submit">${revise ? 'Save facts' : 'Accept and lock facts'}</button>
</form>
<p class="muted">Payment: <a href="${checkout}">Stripe checkout</a></p>
<script>
...
document.getElementById('f').onsubmit = async (e) => {
  ...
  if (data.checkoutUrl && data.next === 'pay_then_awaiting_ken') {
    location.href = data.checkoutUrl;
    return;
  }
  document.body.innerHTML = '<div class="w"><h1>' + (data.status || (data.ok ? 'Locked' : 'Needs a fix')) + '</h1><pre>' + JSON.stringify(data, null, 2) + '</pre></div>';
};
</script>
</div></body></html>`;
}
...
export function registerAutoCloseRoutes(app, { admin } = {}) {
  ...
  app.post('/api/close/respond', async (req, res) => {
    try {
      ...
      res.status(out.ok || out.status === 'awaiting_ken' ? 200 : 400).json(out);
    } catch (e) {
      res.status(500).json({ ok: false, error: e.message, default: 'stop' });
    }
  });
  ...
  app.get('/api/close/status', (_req, res) => {
    res.json({ ok: true, default: 'stop', armed: autoExecuteArmed(), kenPhrase: GO_AHEAD_PHRASE });
  });
  ...
  app.post('/api/ops/close/go-ahead/:jobId', async (req, res) => {
    if (typeof admin === 'function' && !admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    const granted = grantKenGoAhead(req.params.jobId, {
      phrase: req.body?.phrase || req.body?.confirm,
      note: req.body?.note,
    });
    if (!granted.ok) return res.status(400).json(granted);
    if (req.body?.execute === true) {
      try { return res.json({ granted, execute: await executeJob(req.params.jobId) }); }
      catch (e) { return res.status(500).json({ granted, error: e.message, default: 'stop' }); }
    }
    res.json(granted);
  });

  app.post('/api/ops/close/hold/:jobId', (req, res) => {
    if (typeof admin === 'function' && !admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    res.json(revokeKenGoAhead(req.params.jobId, req.body?.note || 'hold'));
  });
