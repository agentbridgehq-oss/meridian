...
import { BASE, listAgents, ensureWidgetToken, verifyAgentKey } from '../engine.mjs';
...
export function buildSetupContext({ agentId, apiKey, businessName, base, deliveryToken, widgetToken }) {
  ...
  return {
    ...
    widgetToken: wt,
    widgetSnippet: id && wt
      ? `<script src="${b}/widget.js" data-agent="${id}" data-token="${wt}" data-name="${String(businessName || 'Assistant').replace(/"/g, '')}"></script>`
      : null,
    endpoints: id
      ? {
