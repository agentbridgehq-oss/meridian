...
export function buildSetupContext({ agentId, apiKey, businessName, base, deliveryToken, widgetToken }) {
  ...
  return {
    ...
    widgetSnippet: id && wt
      ? `<script src="${b}/widget.js" data-agent="${id}" data-token="${wt}" data-name="${escAttr(businessName || agent?.businessName || 'Assistant')}"></script>`
      : null,
    endpoints: id
      ? {
...
export async function processInstallJob(job, { sendEmail } = {}) {
  ...
  const pack = {
    widgetSnippet: wt
      ? `<script src="${base}/widget.js" data-agent="${job.agentId}" data-token="${wt}" data-name="${escAttr(job.businessName)}"></script>`
      : null,
    platforms,
    n8nWorkflow: n8n,
