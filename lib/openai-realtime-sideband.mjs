...
export async function connectOpenAIRealtimeSideband({
  ...
  if (!/^rtc_[A-Za-z0-9_-]+$/.test(String(callId || ''))) {
    return { ok: false, code: 'invalid_call_id', error: 'A valid Realtime call ID is required.' };
  }
  if (!deploymentId) return { ok: false, code: 'deployment_missing', error: 'deploymentId is required.' };

  let rt;
  try {
    rt = await realtimeFactory(callId);
  } catch (error) {
    return { ok: false, code: error?.code || 'sideband_connect_failed', error: safeError(error) };
  }
  if (!rt || typeof rt.send !== 'function') {
    return { ok: false, code: 'invalid_sideband_client', error: 'Realtime sideband factory did not return a usable client.' };
  }
  ...
  cleanups.push(onRealtime(rt, 'response.output_item.added', event => {
    void handleToolEvent(event);
  }));
  cleanups.push(onRealtime(rt, 'response.function_call_arguments.done', event => {
    void handleToolEvent(event);
  }));
  ...
  cleanups.push(onRealtime(rt, 'error', error => {
    lastError = safeError(error);
  }));

  cleanups.push(onSocket(rt.socket, 'open', () => {
    controller.markConnected('Official OpenAI Node SDK sideband WebSocket opened.');
  }));
  cleanups.push(onSocket(rt.socket, 'close', () => {
    ...
    controller.markEnded({
      ...
      detail: lastError ? 'Realtime sideband closed after an error.' : 'Realtime sideband closed.',
      error: lastError,
    });
  }));
