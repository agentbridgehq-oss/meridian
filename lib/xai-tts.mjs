...
/** Premium default: Ara — warm, human receptionist (override with XAI_TTS_VOICE) */
...
/** Ordered fallbacks when primary voice fails — human-first, never silent */
const VOICE_FALLBACK_CHAIN = String(
  process.env.XAI_TTS_FALLBACK_VOICES || 'ara,eve,carina,luna,orion,rex,sal',
)
  .split(',')
  .map((s) => s.trim().toLowerCase())
  .filter(Boolean);
const TTS_RETRIES = Math.max(1, Number(process.env.XAI_TTS_RETRIES || 3));
const TTS_TIMEOUT_MS = Number(process.env.XAI_TTS_TIMEOUT_MS || 45000);
/**
 * Overall wall-clock budget across ALL voices/retries combined. Without this,
 * a degraded (not fully down) xAI endpoint could burn up to TTS_RETRIES ×
 * VOICE_FALLBACK_CHAIN.length × TTS_TIMEOUT_MS (worst case: 3 × 7 × 45s =
 * 15+ minutes) of dead air on a live call before ever reaching the
 * ElevenLabs/text fallback. This caps total time spent here so a live turn
 * degrades quickly instead of hanging.
 */
const TTS_TOTAL_BUDGET_MS = Number(process.env.XAI_TTS_TOTAL_BUDGET_MS || 12000);
 ...
 */
async function xaiTtsOnce(clean, voice, lang, key) {
  let res;
  try {
    res = await fetch(API, {
      method: 'POST',
    ...
    return { ok: false, mode: 'xai', error: e.message || 'xAI TTS network error', voiceId: voice };
  }
  ...
  if (ct.includes('application/json')) {
    try {
      ...
      if (b64) {
        ...
        if (buf.length < 200) {
          return { ok: false, mode: 'xai', error: 'xAI TTS audio too short', voiceId: voice };
        }
        return {
          ok: true,
          mode: 'xai',
    ...
    return { ok: false, mode: 'xai', error: 'xAI TTS returned JSON without audio', voiceId: voice };
  }

  const buf = Buffer.from(await res.arrayBuffer());
  if (!buf.length || buf.length < 200) {
    return { ok: false, mode: 'xai', error: 'xAI TTS returned empty/short audio', voiceId: voice };
  }
...
function voiceAttemptList(preferred) {
  const primary = String(preferred || DEFAULT_VOICE)
    .toLowerCase()
    .trim()
    .slice(0, 64);
  const list = [primary, ...VOICE_FALLBACK_CHAIN.filter((v) => v !== primary)];
  return [...new Set(list)];
}

/**
 * Premium TTS with retries + voice fallback chain.
 * Never throws. Tries alternate human voices if primary fails.
 *
 * @returns {Promise<{ ok: boolean, mode?: string, audioBuffer?: Buffer, contentType?: string, voiceId?: string, chars?: number, error?: string, httpStatus?: number, attempts?: number }>}
 */
export async function xaiTextToSpeech(text, { voiceId, language } = {}) {
  ...
  const voices = voiceAttemptList(voiceId || process.env.XAI_TTS_VOICE || DEFAULT_VOICE);
  const errors = [];
  let attempts = 0;
  const startedAt = Date.now();
  let budgetExceeded = false;

  outer: for (const voice of voices) {
    for (let r = 0; r < TTS_RETRIES; r++) {
      if (Date.now() - startedAt >= TTS_TOTAL_BUDGET_MS) {
        budgetExceeded = true;
        break outer;
      }
      attempts += 1;
      const result = await xaiTtsOnce(clean, voice, lang, key);
      if (result.ok && result.audioBuffer) {
        try {
          const { recordVendorPayg } = await import('./vendor-payg.mjs');
          recordVendorPayg('xai_tts', {
            ok: true,
            meta: { voiceId: result.voiceId, chars: clean.length, attempts },
          });
        } catch {
        ...
        return {
          ...result,
          attempts,
          fallbackUsed: voice !== voices[0],
          billing: 'pay_as_you_go',
        };
      }
      errors.push(`${voice}#${r + 1}: ${result.error || 'fail'}`);
      // brief backoff before retry same voice
      if (r < TTS_RETRIES - 1) {
        await new Promise((res) => setTimeout(res, 180 * (r + 1)));
      }
    }
  }
  ...
  try {
    const { recordVendorPayg } = await import('./vendor-payg.mjs');
    recordVendorPayg('xai_tts', { ok: false, meta: { attempts, errors: errors.slice(-3) } });
  } catch {
    /* ignore */
  }
  ...
  return {
    ...
    error: budgetExceeded
      ? `xAI TTS gave up after ${attempts} attempt(s) — exceeded ${TTS_TOTAL_BUDGET_MS}ms total budget (last errors: ${errors.slice(-2).join(' | ') || 'none yet'})`
      : `xAI TTS failed after ${attempts} attempts: ${errors.slice(-4).join(' | ')}`,
    attempts,
    triedVoices: voices,
    budgetExceeded,
    billing: 'pay_as_you_go',
...
/** Alias for clarity in skills/docs */
export const xaiTextToSpeechReliable = xaiTextToSpeech;
