...
const OPTIONAL_SERVICE_INTEGRATIONS = Object.freeze({
  voice: ['calendar'],
});
...
function integration(kind, required = true) {
  return { kind, required: required === true, requiresCredential: !NO_CREDENTIAL_REQUIRED.has(kind), provider: '', status: 'pending', credentialConfigured: false, externalId: '', endpoint: '', evidence: '', lastCheckedAt: null, updatedAt: now() };
}
export function optionalIntegrationKinds(service) {
  return [...(OPTIONAL_SERVICE_INTEGRATIONS[service] || [])];
}
function allowedIntegrationKinds(service, agentNeed = '') {
  return [...new Set([...requiredIntegrationKinds(service, agentNeed), ...optionalIntegrationKinds(service)])];
}
...
function evaluate(d) {
  const xsAll = Object.values(d.integrations || {}), xs = xsAll.filter(x => x.required !== false), cs = d.checks || [], b = blockers(d);
...
export function createDeploymentFromAgencyLead(lead) {
  ...
  const p = lead.agency.proposal || {}, service = clean(p.service || lead.agency.input?.service || lead.primaryNeed,60), need = clean(p.agentNeed || '',40), kinds = requiredIntegrationKinds(service, need), optional = optionalIntegrationKinds(service).filter(k => !kinds.includes(k)), at = now();
  const d = evaluate({ id:`dep_${crypto.randomBytes(10).toString('hex')}`, projectId:lead.id, leadId:lead.id, businessName:clean(lead.businessName,160), service, tier:clean(lead.agency.input?.tier || p.tier,40), capabilities:capabilities(service,need), status:'draft', revision:1,
    ...
    integrations:Object.fromEntries([...kinds.map(k => [k,integration(k,true)]), ...optional.map(k => [k,integration(k,false)])]), checks:(p.acceptanceChecks || []).map((label,i) => ({ id:`check_${i+1}`,label:clean(label,500),status:'pending',evidence:'',checkedBy:'',checkedAt:null })),
...
export function updateIntegration(id, kind, patch = {}, expected = null) {
  ...
  return mutate(id, d => {
    ...
    if (!d.integrations[kind]) {
      const allowed = allowedIntegrationKinds(d.service, d.config?.agent?.need);
      if (!allowed.includes(kind)) return {ok:false,status:404,error:'Integration is not required for this deployment'};
      d.integrations[kind] = integration(kind, requiredIntegrationKinds(d.service, d.config?.agent?.need).includes(kind));
    }
    const x = d.integrations[kind];
