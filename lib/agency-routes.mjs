...
import { registerVoiceDemoRoutes } from './voice-demo-routes.mjs';
import { createDeploymentFromAgencyLead, deploymentSummary } from './deployment-core.mjs';
...
export function registerAgencyRoutes(app, { admin, publicLimiter, checkFormBot, rejectObviousBots }) {
  ...
  registerVoiceDemoRoutes(app, { publicLimiter });
  ...
  app.patch('/api/ops/agency/projects/:id', (req, res) => {
    ...
    // Crossing design -> build creates the durable deployment ledger exactly once.
    // If persistence fails, do not advance the commercial workflow and pretend build started.
    let deployment = null;
    if (target === 'build') {
      const provision = createDeploymentFromAgencyLead(lead);
      if (!provision.ok) return res.status(provision.status || 500).json({ error: provision.error || 'Deployment provisioning failed.' });
      deployment = deploymentSummary(provision.deployment);
    }
    ...
    res.json({ ok: true, project: publicProject(getLead(lead.id)), ...(deployment ? { deployment } : {}) });
  });
}
