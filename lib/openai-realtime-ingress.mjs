...
async function bestEffortReject(options, plan) {
...
export async function processVerifiedOpenAIRealtimeWebhook(event, options = {}) {
  ...
  if (!plan.canAccept) {
    ...
    audit.updateCall(plan.callId, {
      ...
      detail: rejected
        ? 'Inbound call was rejected at OpenAI after failing Meridian readiness gates.'
  ...
  try {
    ...
    audit.updateCall(plan.callId, {
      ...
      detail: rejected
        ? 'OpenAI Realtime call acceptance failed and Meridian declined the still-pending call.'
        : 'OpenAI Realtime call acceptance failed after Meridian authorization.',
    });
    return {
      ok: false,
      status: 502,
