...
export function realtimeToolDefinitions(deployment) {
  ...
  const calendarToolsAllowed = adapterReady(deployment, 'calendar') && (
    deployment.capabilities?.includes('booking') || deployment.capabilities?.includes('voice')
  );
  if (calendarToolsAllowed) {
    tools.push({
      type: 'function',
    ...
    tools.push({
      type: 'function',
      name: 'meridian_cancel_appointment',
      description: 'Cancel an existing appointment through the verified customer calendar adapter. Only report success when the customer system explicitly confirms the cancellation.',
      parameters: {
        type: 'object', additionalProperties: false,
        properties: {
          booking_id: { type: 'string' }, start_time: { type: 'string' }, caller_name: { type: 'string' },
          callback_number: { type: 'string' }, reason: { type: 'string' },
        },
        required: ['booking_id'],
      },
    });
    tools.push({
      type: 'function',
      name: 'meridian_reschedule_appointment',
      description: 'Move an existing appointment through the verified customer calendar adapter after the caller confirms the new slot.',
      parameters: {
        type: 'object', additionalProperties: false,
        properties: {
          booking_id: { type: 'string' }, caller_name: { type: 'string' }, callback_number: { type: 'string' },
          service: { type: 'string' }, start_time: { type: 'string' }, end_time: { type: 'string' }, timezone: { type: 'string' },
          caller_confirmed_slot: { type: 'boolean' }, consent_to_confirmation: { type: 'boolean' },
        },
        required: ['booking_id','start_time','timezone','caller_confirmed_slot'],
      },
    });
  }
...
export function executeRealtimeTool({ deploymentId, name, arguments: args = {} } = {}) {
  ...
  if (name === 'meridian_cancel_appointment') {
    if (!adapterReady(deployment, 'calendar')) return { ok: false, code: 'calendar_adapter_unavailable', message: 'Calendar adapter is not ready.' };
    const bookingId = clean(args.booking_id, 300);
    if (!bookingId) return { ok: false, code: 'invalid_arguments', message: 'Booking id is required to cancel.' };
    return executeBusinessSystemAction({ deployment, kind: 'calendar', action: 'cancel_appointment', data: {
      bookingId, startTime: clean(args.start_time, 120), callerName: clean(args.caller_name, 200),
      callbackNumber: clean(args.callback_number, 100), reason: clean(args.reason, 1000),
    }}).then(result => {
      const interaction = recordToolInteraction({ deployment, agentId, role: 'booking_cancel', message: bookingId,
        reply: result.ok ? 'Customer calendar confirmed the cancellation.' : result.message,
        meta: { bookingId, actionConfirmed: result.confirmed === true }, ok: result.ok });
      return { ...result, action: result.ok ? 'cancel_confirmed' : undefined, interactionId: interaction?.id };
    });
  }

  if (name === 'meridian_reschedule_appointment') {
    if (!adapterReady(deployment, 'calendar')) return { ok: false, code: 'calendar_adapter_unavailable', message: 'Calendar adapter is not ready.' };
    if (args.caller_confirmed_slot !== true) return { ok: false, code: 'slot_not_confirmed', message: 'Caller must confirm the exact new slot before rescheduling.' };
    const bookingId = clean(args.booking_id, 300), startTime = clean(args.start_time, 120), timezone = clean(args.timezone, 120);
    if (!bookingId || !startTime || !timezone) return { ok: false, code: 'invalid_arguments', message: 'Booking id, start time and timezone are required.' };
    return executeBusinessSystemAction({ deployment, kind: 'calendar', action: 'reschedule_appointment', data: {
      bookingId, callerName: clean(args.caller_name, 200), callbackNumber: clean(args.callback_number, 100),
      service: clean(args.service, 300), startTime, endTime: clean(args.end_time, 120), timezone,
      callerConfirmedSlot: true, consentToConfirmation: args.consent_to_confirmation === true,
    }}).then(result => {
      const interaction = recordToolInteraction({ deployment, agentId, role: 'booking_reschedule', message: `${bookingId} -> ${startTime}`,
        reply: result.ok ? 'Customer calendar confirmed the reschedule.' : result.message,
        meta: { bookingId, start: result.start || startTime, actionConfirmed: result.confirmed === true }, ok: result.ok });
      return { ...result, action: result.ok ? 'reschedule_confirmed' : undefined, interactionId: interaction?.id };
    });
  }
