...
import { expertiseFor, resolveAgentRole } from './expertise.mjs';
...
function businessInstructions(deployment, lead) {
  ...
  const role = resolveAgentRole(agent.primaryNeed || profile.primaryNeed, deployment.capabilities);
  return [
    expertiseFor(agent.primaryNeed || profile.primaryNeed, deployment.capabilities),
    '',
    `You are the Meridian ${role} voice agent for ${business}.`,
...
export function buildOpenAIRealtimeConfig(deploymentId) {
  ...
  const role = resolveAgentRole(agent.primaryNeed || deployment.config?.profile?.primaryNeed, deployment.capabilities);
  return {
    ok: true,
    ...
    verification: {
      ...
      requiredChecks: [
        ...
        'Role playbook for receptionist, booking, or service is present in session instructions.',
      ],
    },
    safety: {
