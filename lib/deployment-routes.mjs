...
import { buildDeploymentManifest, providerCatalog, validateProvider } from './provider-registry.mjs';
import { buildOpenAIRealtimeConfig } from './openai-realtime-config.mjs';
import { buildCoreReadinessReport } from './core-readiness.mjs';
import {
  listInboundRoutes,
  setInboundRouteEnabled,
  upsertInboundRoute,
} from './inbound-routing.mjs';
import {
  getRealtimeCall,
  listRealtimeCalls,
  realtimeCallSummary,
...
function sendRouteResult(res, result, successStatus = 200) {
  if (!result?.ok) {
    return res.status(result?.status || 400).json({
      ok: false,
      error: result?.error || 'Inbound route operation failed',
...
export function registerDeploymentCoreRoutes(app, { admin }) {
  app.get('/api/ops/core-readiness', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    return res.json(buildCoreReadinessReport());
  });

  app.get('/api/ops/deployment-providers', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    return res.json({ ok: true, providers: providerCatalog() });
  });

  app.get('/api/ops/inbound-routes', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    return res.json({ ok: true, routes: listInboundRoutes() });
  });

  app.post('/api/ops/deployments/:id/inbound-routes', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    const deployment = getDeployment(req.params.id);
    if (!deployment) return res.status(404).json({ error: 'Deployment not found' });
    return sendRouteResult(res, upsertInboundRoute({
      deploymentId: deployment.id,
      dialedNumber: req.body?.dialedNumber,
      provider: req.body?.provider || 'twilio-sip',
  ...
  app.post('/api/ops/inbound-routes/:routeId/enable', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    return sendRouteResult(res, setInboundRouteEnabled(req.params.routeId, true, {
      evidence: req.body?.evidence,
    }));
  });

  app.post('/api/ops/inbound-routes/:routeId/disable', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    return sendRouteResult(res, setInboundRouteEnabled(req.params.routeId, false, {
      evidence: req.body?.evidence,
    }));
  });

  app.get('/api/ops/realtime-calls', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    const deploymentId = typeof req.query.deploymentId === 'string' ? req.query.deploymentId : '';
    const limit = Number(req.query.limit || 100);
    const calls = listRealtimeCalls({ deploymentId, limit });
    return res.json({
      ok: true,
      calls: calls.map(realtimeCallSummary),
      counts: calls.reduce((acc, call) => {
  ...
  app.get('/api/ops/realtime-calls/:callId', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    const call = getRealtimeCall(req.params.callId);
    if (!call) return res.status(404).json({ error: 'Realtime call not found' });
    return res.json({ ok: true, call, summary: realtimeCallSummary(call) });
  });
  ...
  app.get('/api/ops/deployments/:id/manifest', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    const deployment = getDeployment(req.params.id);
    if (!deployment) return res.status(404).json({ error: 'Deployment not found' });
    const lead = getLead(deployment.projectId);
    return res.json({ ok: true, manifest: buildDeploymentManifest(deployment, lead) });
  });

  app.get('/api/ops/deployments/:id/openai-realtime-config', (req, res) => {
    res.set('Cache-Control', 'no-store');
    if (!admin(req)) return res.status(401).json({ error: 'Unauthorized' });
    const result = buildOpenAIRealtimeConfig(req.params.id);
  ...
  app.patch('/api/ops/deployments/:id/integrations/:kind', (req, res) => {
    ...
    if (req.body?.provider) {
      const provider = validateProvider(req.params.kind, String(req.body.provider));
      if (!provider.ok) return res.status(400).json({ ok: false, error: provider.error });
    }
