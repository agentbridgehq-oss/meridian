...
export const DENY_PATH_PATTERNS = [
  ...
  /\.env(\.|$|local)/i,
  /id_rsa|\.pem$|credentials\.json|(?<![A-Za-z])password(?![A-Za-z])|secret.?key|auth\.json/i,
  // Word boundaries required — bare "ssn" matched inside "businessName" and blocked Full Auto install
  /customer[_\s.-]*(export|csv|dump|pii)|\bssn\b|\bsin\b|tax[_\s.-]*return|\bw-?2\b|\bt4\b/i,
  /onedrive|(?<![A-Za-z])documents(?![A-Za-z])|downloads.*(statement|invoice|tax)|desktop\\.*(password|bank)/i,
...
export function containmentStatus() {
  return {
    ...
    only: [
      ...
      'casl_queue_drafts_from_outreach_queue_json',
    ],
    noDeliverFlag: true,
    humanOnly: [
      'money',
      ...
      'bank',
      'account_passwords',
      'approved_send_cold_outreach',
      'confirm_APPROVED_SEND',
    ],
