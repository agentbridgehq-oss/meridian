 ...
 * Real fallback: if xAI is preferred but fails (after its own retries + voice
 * chain), falls through to ElevenLabs when armed, before giving up to
 * platform text-only — matching the reliability chain documented in
 * PREMIUM-VOICE.md, which previously wasn't actually implemented here (the
 * xAI branch returned unconditionally, so ElevenLabs was unreachable dead code).
 */
  ...
  if (wantAudio && mode !== 'platform') {
    if (mode === 'xai' && xaiTtsConfigured()) {
      ...
      if (tts.ok && tts.audioBuffer) {
        ...
        ttsError = tts.error;
        // xAI exhausted its own retries + voice chain — fall through to
        // ElevenLabs if the customer armed it as a backup, instead of
        // silently dropping to text-only (matches PREMIUM-VOICE.md's
        // documented xAI -> ElevenLabs -> text reliability chain).
        if (elevenlabsEnabled()) {
